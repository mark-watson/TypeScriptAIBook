...
1. **Hyperparameter Tuning**: In [cancer_model.ts](file:///Users/markwatson/GITHUB/TypeScriptAIBook/source-code/deep_learning_basics/cancer_model.ts), modify the learning rate of the Stochastic Gradient Descent (SGD) optimizer from `0.01` to `0.1` and `0.001`. Alternatively, switch the optimizer to Adam using `tf.train.adam(0.001)`. How do these changes affect the training speed (loss convergence) and final test accuracy?
2. **Experimenting with Neural Architectures**: Modify the layer architecture in [buildModel](file:///Users/markwatson/GITHUB/TypeScriptAIBook/source-code/deep_learning_basics/cancer_model.ts#L21-L29). Try adding a third hidden layer or changing the number of `units` in the hidden layers (e.g., 8 or 32 units). Track if a more complex model improves test accuracy or leads to overfitting.
3. **Regularization and Dropout**: The model in [cancer_model.ts](file:///Users/markwatson/GITHUB/TypeScriptAIBook/source-code/deep_learning_basics/cancer_model.ts) uses a dropout layer with a rate of `0.2`. Try setting the rate to `0.0` (disabling it) and `0.5`. Examine the classification report and training loss. What role does dropout play in preventing overfitting on small datasets?
4. **Activation Functions**: The hidden layers currently use the `"relu"` (Rectified Linear Unit) activation function. Rewrite the model compilation to use `"tanh"` or `"sigmoid"` activations for the hidden layers. Compare the training loss curve and test metrics against the ReLU-based network.
5. **Adjusting the Decision Threshold**: In [cancer_model.ts](file:///Users/markwatson/GITHUB/TypeScriptAIBook/source-code/deep_learning_basics/cancer_model.ts#L56), classification predictions are mapped using a fixed threshold of `0.5` (`v >= 0.5 ? 1 : 0`). In medical diagnostic applications, minimizing false negatives (high recall) is often prioritized over precision. Modify the evaluation code to use a threshold of `0.3`. How does this change affect the precision, recall, and F1-score for the positive class (`1.0`) in the classification report?
